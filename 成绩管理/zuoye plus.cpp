...
void Sort(ID *head); //排序 
ID* danke(ID* head);//录入单科成绩 
ID* mmSort(ID* head); //单科成绩、总成绩排序  
/************************************************
*函数名： main(主函数)
*功能：显示主菜单
*返回值：NULL
...
{   
    ...
    if ((fp=fopen("student.txt","r"))==NULL)      //如果文件不存在就创建
    {
        ...
        while(1)                //这个循环可以实现多次添加新学生信息
    	{
			...
			scanf("%d",&p1->age);
			fflush(stdin);
			printf("请输入学生科目成绩(语文，数学，英语，化学，物理，政治，地理，生物，体育，历史)：");
...
{
	ID *t1=head;
	printf("\n学生姓名==学号===性别===年龄==语文成绩==数学成绩==英语成绩==化学成绩==物理成绩==政治成绩==地理成绩==生物成绩==体育成绩==历史成绩==总分==平均分\n\n");
	while(t1!=NULL)
	{
...
ID* mmSort(ID* head)
{
	...
	while(1)
	{
		...
		fflush(stdin);
		printf("请输入您的选择:\n");
		scanf("%d",&num);//选择你想要查询的排序方式 
		switch(num) 
		{
			case 1: 
			{
				struct student temp;//定义结构体变量temp。
				int t;
			...
			case 12:
				{
					printf("你输入错了，请你输入任意键返回主菜单...\n");
    				getch();
				}	
		}
	}
	Save(head);
	printf("学生已按照 您的选择 排序成功,是否存盘?Y/N?\n");
	fflush(stdin);
	scanf("%c",&saveflag);//确定要保存吗？
	if(saveflag=='y'||saveflag=='Y')//当输入是的时候
	Save(head); //调用保存函数
